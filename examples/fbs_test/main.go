...
func main() {
	/*
		...
		offset := requestData.Pack(builder)
		builder.Finish(offset)
		datas := builder.FinishedBytes()

		newRequestFBS := FBSRequest.GetSizePrefixedRootAsRequest(datas, 0)
		newRequest := newRequestFBS.UnPack()
		println("routerId=", newRequest.Body)
	...
	*/
	_, err = worker.CreateRouter(mediasoup.RouterOptions{
